...
- 密码从 `PasswordField` 取得后立即清空控件，转换的 `char[]` 由门面消费并清零；若后台任务尚未开始就关闭窗口，待处理密码数组也会主动清零；
...
- 完整 `verify` 通过：16 个测试套件、46 个测试，0 failure、0 error、0 skipped。
...
- JavaFX `PasswordField` 内部以不可清零的 `String` 保存当前文本；应用会尽快清空控件并清零自己创建的 `char[]`，但不能声称清除了 JavaFX/JVM 内部副本；
