...
- 文档状态：阶段 1、阶段 2 已完成；阶段 3 已获用户确认并开始开发
...
- 阶段 1 起始提交为 `15677e7`；阶段 2 受控私钥导出与编码测试完成后，当前固定 `HEAD` 为 `4a5e9a747148fd05940e7571ff4cc80f014a4127`。
...
- 2026-07-23 使用 JDK `17.0.16` 和 Maven `3.9.9` 通过桌面准备脚本重新执行完整核心测试：96 个测试套件、933 个测试，0 failure、0 error、0 skipped。
...
- 桌面端已把固定依赖从 `15677e7` 切换到 `4a5e9a7`；核心完整测试、隔离 Maven 安装和桌面 `verify` 均已通过，可以开始 Vault 测试先行实现。
...
阶段 2 已完成并经用户确认，当前进入阶段 3：

1. 保持核心基线 `4a5e9a7` 和阶段 2 Vault schema 不变；
2. 先建立 Vault 到真实核心发送门面的业务层；
3. 再接入独立 JavaFX 发送页、文件输入和三种输出；
4. 完成真实组合矩阵、Windows UI smoke 和阶段 3 完成报告后停下等待确认。

阶段 3 实时范围、门禁和闭环记录见
`docs/dev/05-phase-3-send-implementation.md`。
