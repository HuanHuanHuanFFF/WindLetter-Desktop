...
- X25519、ML-KEM-768、Ed25519 私钥 handle 支持生成、导入、使用和销毁，但不支持受控导出私钥材料。
- 核心门面会取得私钥 lease 的所有权并在一次操作后关闭 handle；可持久化实现必须在每次 `open` 时从受保护存储重新导入新的短生命周期 handle。
- 因此阶段 1 的内存密钥闭环可直接实现，但阶段 2 的“生成后持久化、备份、导出、重新解锁”不能仅靠当前公开 API 完成。不得在桌面端通过反射、复制 provider 实现或另造 ML-KEM 密钥编码绕过这一边界。
...
## 6. 私钥存储暂定边界（设计提案，尚未授权实施）

### 6.1 威胁模型

拟防护：应用数据目录、备份或磁盘被离线复制；普通日志/崩溃信息意外泄漏；Vault 文件被静态篡改。
...
- 使用 AES-256-GCM 加密私钥记录；每次写入使用唯一随机 nonce；AAD 绑定格式版本、身份 ID、key type 和 KID。
...
以上方案必须在阶段 2 开始前由用户确认，并以核心库新增的受控私钥边界为前置条件。
...
| 等级 | 项目 | 影响 | 处理 |
|---|---|---|---|
| P0（阶段 2） | 核心 handle 无受控私钥导出/序列化 | 无法安全实现持久化、备份和 Hybrid 身份恢复 | 在核心仓库单独设计、实现、测试并固定新基线；桌面不得绕过 |
...
| P1（阶段 2） | 公共身份/联系人交换格式未定义 | 导入导出可能互不兼容 | 阶段 2 先冻结版本化、严格校验的产品层格式，不冒充 WindLetter 协议 |
...
1. 同意阶段 1 固定主流程为 `PUBLIC + X25519 + signed + BASE64_PEM`，其他组合先由集成测试覆盖。
2. 同意采用 JavaFX `21.0.10` + Java 17 + 非 JPMS/classpath 起步。
3. 同意核心库先采用“完整 SHA 校验 + 隔离本地 Maven 安装”的过渡依赖方案。
4. 确认 Git 推送方式：修复 SSH 公钥，或允许继续用 HTTPS remote/凭据。
5. 知悉阶段 2 在核心私钥序列化边界解决前是阻塞状态；本轮不会实现或落盘真实私钥。

收到确认后，只开始阶段 1，不提前实现身份 Vault、完整发送/接收界面或发布打包。
