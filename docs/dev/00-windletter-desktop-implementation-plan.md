...
- 文档状态：阶段 1、阶段 2、阶段 3 已完成；等待用户确认阶段 4
...
1. Vault 到真实核心发送门面的业务层已完成；
2. 独立 JavaFX 发送页、文件输入、多收件人和三种输出已完成；
3. 真实组合矩阵、Windows JavaFX `Stage` smoke 和失败边界已通过；
4. 不提前进入接收页。

阶段记录见 `docs/dev/05-phase-3-send-implementation.md`，
