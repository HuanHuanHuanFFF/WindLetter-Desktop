...
- 当前状态：实现完成、自动化验证完成、运行入口已接入；等待交互式桌面 UI smoke 后关闭阶段
...
Codex 工具进程位于隔离的非交互桌面会话。两次 Windows UI Automation 尝试都无法发现顶层窗口，因此本记录不把后台 Java 进程存在冒充为“按钮已点击成功”。交互式 UI smoke 仍需在用户桌面会话执行：
...
在交互式 smoke 成功前，阶段 1 状态是“候选完成”，不是“已完整验收”。
...
| UI smoke 依赖交互式 Windows 桌面 | 当前隔离 Codex 会话不能完成控件级验收 | 用户桌面执行脚本；阶段 5 接入可交互 Windows CI |
...
这些 P2 不改变当前协议、密码学或认证正确性证据，但交互式 UI smoke 是关闭阶段 1 前必须补齐的验证项。
